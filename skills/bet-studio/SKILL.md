...
| **Research a bet** | *Phase 4 `bet-research` — not built.* Interim: bet-studio stays read-only and self-contained — one action, not a tutorial. From a plain-English description of the conversation (e.g. "talked to the career services director at Mott about outcomes data — she asked what it would cost"), build the ENTIRE `log_evidence` call in one pass — `kind`, `signal_strength`, `entity_type`/`entity_id` (from the bet's targets, or ask once), `data.problem_confirmed` (judge it from the description, CONFIRM with the human), `link` (request it) — fully filled in; draft-don't-fire holds, the human fires it themselves. Always follow the draft by reporting the bet's progress against the research gate as a text bar computed from `get_bet`'s evidence list: `conversations: 3/5 · distinct institutions: 2/3 · problem-confirmed: 3/4 · exploration+ w/ link: 1/2`. `bet-idea` creates or RESUMES Idea-stage bets; it won't operate on bets already at Research+. For the interview method itself: the Customer Forces method lives in `bet-idea`'s reference material — consult it, but the call is drafted here. | Idea → Research |
