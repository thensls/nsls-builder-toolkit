...
- Every `href` resolves. **Branch on the form of the value first** — only `http(s)` has a status code to quote, and every branch has its own pass/fail, not just the fetched one:
  ...
  - **`mailto:` / `tel:` / `sms:`** — no fetch, no status, but they still pass or fail. Parse the payload: `mailto:` needs a syntactically valid address (and check the domain looks real, not `example.com`), `tel:`/`sms:` need a dialable number (E.164-ish; digits, optional leading `+`, no letters). A malformed one — `mailto:not-an-address`, `tel:abc`, an empty payload — is a **`QA.links` finding at the same severity as a 404**, because it is equally dead when tapped. What you must not do is report a *well-formed* one as broken merely because you couldn't fetch it.
