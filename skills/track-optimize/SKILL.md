...
- `references/verifying-generate-screens.md` — REQUIRED before calling any track with `generate` screens ready: the `/api/generate` call shape, the grounding-join rules that fail silently, how to diff printed figures against the snapshot in code, the empty-grounding case, and authoring rules learned by measured failure. The panel scores a transcript of intent and cannot see invented figures, unresolved joins, or unsaved answers.
