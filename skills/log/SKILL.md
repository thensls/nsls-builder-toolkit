...
Run `/obsidian-log` at the end of a working session (or any point mid-session) to capture:
...
The active Obsidian vault is at:
```
~/Library/Mobile Documents/iCloud~md~obsidian/Documents/KP/
```

Project logs live at:
```
20-projects/[project-slug]/sessions/YYYY-MM-DD.md
```

Each project also has a **home note** at:
```
20-projects/[project-slug]/[project-slug].md
```
...
| Context | Project slug | Obsidian path |
|---|---|---|
| `~/claude-skills/people-ops/` | `people-ops` | `20-projects/people-ops` |
| `~/claude-skills/slt-ops/slt-bot/` | `slt-bot` | `20-projects/slt-bot` |
| `~/claude-skills/slt-ops/slt-meeting-agenda/` | `meeting-automation` | `20-projects/meeting-automation` |
| `~/claude-skills/slt-ops/` | `slt-ops` | `20-projects/meeting-automation` |
| `~/claude-skills/product/` | `posthog-analytics` | `20-projects/posthog-analytics` |
| `~/.claude/skills/email-triage/` | `personal-productivity` | `20-projects/personal-productivity` |
| `~/.claude/skills/nsls-slides/` | `personal-productivity` | `20-projects/personal-productivity` |
| `~/.claude/skills/google-docs/` | `personal-productivity` | `20-projects/personal-productivity` |
| `~/.claude/skills/netlify-deploy/` | `personal-productivity` | `20-projects/personal-productivity` |
| `~/.claude/skills/frontend-slides/` | `personal-productivity` | `20-projects/personal-productivity` |
| `~/.claude/skills/log/` or `~/.claude/hooks/` | `obsidian` | `20-projects/obsidian` |

For any project not in this table, ask Kevin for the slug — or treat it as a new project.
...
Check (in order):
1. **Explicit name** — did Kevin name the project in the invocation? (e.g., `/obsidian-log for people-ops`)
2. **Working directory** — run `pwd` and match against the known mappings table above
3. **Conversation context** — scan recent messages for directory paths, skill names, project names
4. **Ask** — if still ambiguous, present a short list of likely candidates + "New project" option

If Kevin says **"new project"** (or names something not in the vault yet) → go to **New Project flow** (Step 2A).
...
Create `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/KP/20-projects/[slug]/[slug].md`:
...
Create `~/Library/Mobile Documents/iCloud~md~obsidian/Documents/KP/20-projects/[slug]/sessions/YYYY-MM-DD.md`:
...
**2A-4. Register in obsidian-projects.json**

Add the new project to `~/.claude/obsidian-projects.json`:
```json
"[slug]": {
  "dirs": ["[filesystem path that would be edited for this project]"],
  "obsidian": "20-projects/[slug]"
}
```

**2A-5. Confirm and write**
Show Kevin the draft home note and first session log. Write after he confirms.
...
Present the draft session log to Kevin. Write after he approves (or edits inline).
...
Add a `[[sessions/YYYY-MM-DD|YYYY-MM-DD]]` wikilink to the `## Sessions` section of the project home note (create the section if it doesn't exist). Insert newest dates first (reverse chronological). This keeps sessions connected in Obsidian's graph view.
...
Confirm: "Logged to `20-projects/[slug]/sessions/YYYY-MM-DD.md`" and show the session log so Kevin can verify it landed right.
