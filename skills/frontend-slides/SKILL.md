...
### Optional: Publish to Netlify

If the user wants to publish the presentation online, invoke the **netlify-deploy** skill:

```
/netlify-deploy
```

The netlify-deploy skill will:
1. Copy the HTML file as `index.html` into a temp deploy directory
2. Deploy to Netlify via CLI (with correct content-type — avoids the API zip pitfall)
3. Run a Playwright visual review of all slides at 1440×900
...
- **netlify-deploy** — Publish the finished presentation to Netlify + run Playwright visual review. Chain this after delivery for production-ready output. Handles CLI version gotchas, auth, content-type pitfalls, and screenshot-based QA.
