...
KNOWN_KEYS = {"id", "name", "subject", "preheader", "body_html"}
IGNORABLE = {"esp", "source", "note", "notes", "url", "template_id",
             "action_id", "campaign", "campaign_id", "from_name", "send_time"}
...
def compare(arm_a, arm_b):
    ta, tb = prose_blocks(arm_a.get("body_html")), prose_blocks(arm_b.get("body_html"))
    la, lb = links(arm_a.get("body_html")), links(arm_b.get("body_html"))
    ca, cb = ctas(arm_a.get("body_html")), ctas(arm_b.get("body_html"))
    ...
    return {
        ...
        "detail": elements,
        "attribution_allowed": len(elements) == 1,
    }
