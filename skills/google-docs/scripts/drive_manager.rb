...
class DriveManager
  ...
  def authorize
    ...
    if credentials.nil?
      ...
      output_json({
        ...
        message: 'Authorization required. Visit the auth_url to authorize, then re-run.',
        auth_url: url
      })
  ...
  def upload(file_path:, folder_id: nil, name: nil, mime_type: nil)
  ...
  # Upload a file and convert it to a native Google format
  # e.g., .docx → Google Doc, .xlsx → Google Sheet
  def upload_and_convert(file_path:, folder_id: nil, name: nil)
    unless File.exist?(file_path)
      output_json({
        status: 'error',
        error_code: 'FILE_NOT_FOUND',
    ...
    file_name = name || File.basename(file_path, File.extname(file_path))
    source_mime = detect_mime_type(file_path)

    # Determine target Google format based on source extension
    target_mime = case File.extname(file_path).downcase
                  when '.docx'
                  ...
                  when '.pptx'
                    ...
                    output_json({
                      ...
                      operation: 'upload_and_convert',
                      message: "Cannot auto-convert #{File.extname(file_path)} files. Supported: .docx, .xlsx, .pptx"
                    })
                    exit EXIT_OPERATION_FAILED
                  end

    # Setting file_metadata.mime_type to a Google Apps type while uploading
    # a non-Google file triggers automatic conversion
    file_metadata = Google::Apis::DriveV3::File.new(
      name: file_name,
  ...
  # Resolve a folder path, creating missing segments as needed
  # e.g., "SLT Meetings/2026/February" → finds or creates each level, returns leaf folder ID
  def resolve_folder(path:, root_folder_id: nil)
    ...
    current_parent = root_folder_id || 'root'
    resolved = []

    segments.each do |segment|
      # Search for existing folder with this name under current parent
      query = "name = '#{segment.gsub("'", "\\\\'")}' and '#{current_parent}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
      ...
      if results.files && !results.files.empty?
        # Folder exists
        folder = results.files.first
        current_parent = folder.id
        resolved << { name: segment, id: folder.id, created: false }
      else
        # Create the folder
        ...
        current_parent = folder.id
        resolved << { name: segment, id: folder.id, created: true }
      end
    end
    ...
    output_json({
      ...
      web_view_link: "https://drive.google.com/drive/folders/#{current_parent}",
      segments: resolved
    })
  rescue Google::Apis::Error => e
    output_json({
...
def usage
  puts <<~USAGE
    ...
    Commands:
      ...
      upload-and-convert  Upload and convert to Google format (.docx→Doc, .xlsx→Sheet)
      resolve-folder  Find or create nested folder path (e.g., "A/B/C")

    Options:
      --file <path>       Local file path (for upload/update/upload-and-convert)
      --file-id <id>      Drive file ID
      --folder-id <id>    Drive folder ID
      --output <path>     Output file path (for download)
      --name <name>       File/folder name
      --path <path>       Folder path for resolve-folder (e.g., "SLT Meetings/2026/Feb")
    ...
    Examples:
      ...
      #{File.basename($PROGRAM_NAME)} upload-and-convert --file ./report.docx --folder-id 1abc123 --name "Q1 Report"

      # Resolve/create nested folder path
      #{File.basename($PROGRAM_NAME)} resolve-folder --path "SLT Meetings/2026/February"
  ...
  when 'resolve-folder'
    unless options[:path]
      puts JSON.pretty_generate({
        ...
        message: 'Folder path required: --path <path>'
      })
      exit DriveManager::EXIT_INVALID_ARGS
    end

    manager.resolve_folder(
      path: options[:path],
      root_folder_id: options[:folder_id]
    )
